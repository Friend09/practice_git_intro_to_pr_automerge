...
- **`GITHUB_TOKEN`:** A token GitHub mints automatically for every workflow run, scoped by the
  `permissions:` block, and destroyed when the run ends.
...
the moment the run ends (or shortly after, per GitHub's own retention window). There is no way to
extend its lifetime past one run.
...
- **`GITHUB_TOKEN` is minted per-run, scoped by `permissions:`, and destroyed at run's end** — no
  configuration extends its lifetime.
